  ...
  If lPos <> 0
   ...
   // Формируем текст, если он есть
    ...
    If lPos <> 0
     ...
     End ;
    // БАГ!!! tStr НЕ ДОЛЖЕН СОДЕРЖАТЬ НИЧЕГО. КРОМЕ ДОМЕНА ПЕРВОГО УРОВНЯ!!!
    lPos := Pos( '/', tStr ) ;
    If lPos <> 0
     Then
      tStr := Copy( tStr, 1, lPos - 1 ) ;
    If (Pos(tStr,tUrl)<>0) AND (Pos( tStr, tUrl ) < 9)
     Then Begin FLocal.Add( tUrl ) ; Inc( FCntLocal ) ; End
     Else Begin
      FInterURL.Add( tUrl+'='+tAnchor ) ;
...
Procedure TCSIParser.Execute ;
 ...
 Begin
  ...
  // Проверяем. Если это http, это одна песня, если xml - другая...
