 ...
 Begin
   ...
   If tRes And (lHttp.ResultCode = 200)
    Then Begin
     ...
     lPars.Free ;
     // Убираем дубликаты - xxx.ru xxx.ru/ - это одно и тоже...
{     While List_LocalURL.Find( lUrl, I ) Do
      List_LocalURL.Delete( I );

     While List_LocalURL.Find( lUrl+'/', I ) Do
      List_LocalURL.Delete( I ); }

     List_LocalURL.Sorted := False ;
     For I := 0 To List_LocalURL.Count - 1 Do
      Begin
       S := List_LocalURL.Strings[I] ;
       S := Trim ( S ) ;
       List_LocalURL.Strings[I] := S ;
      End ;
...
//   CheckSynchronize ;
  ...
  AssignFile( F, GetCurrentDir+'\debug\'+'err.csi'); Rewrite( F ) ;
  For I := 0 To vBPError.vList.Count - 1 Do
   Begin
    wErrRec := PErrRec( vBPError.vList.Items[I] ) ;
    WriteLn( F, wErrRec^.FURL ) ;
   End ;
  CloseFile( F ) ; 
 End;
{------------------------------------------------------------------------------}
